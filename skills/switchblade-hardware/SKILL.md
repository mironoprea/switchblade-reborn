...
3. Use `--backend usb` only when MI_03 is bound to WinUSB. That direct path renders the main touch LCD on OUT `0x01` and physical key images on OUT `0x02` when the key endpoint is present.
...
5. Run `pytest` before handoff. Current expected baseline is the full local suite passing.
...
- Direct key images use the SDK-traced `KEY_RECTS`, endpoint `0x02`, and 115x115 RGB565 payloads.
